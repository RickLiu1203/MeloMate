...
interface TracksProps {
  ...
  handleClick: ((id: string) => void) | boolean;
  text: string;
  seeds: string[];
  clickText: string;
  isRecommend: boolean;
            ...
            <div className="flex flex-row justify-between px-4 items-center" key={track.id}>
              <div className="flex flex-row w-3/4 me-2">
                <img
                  ...
                  className="md:h-[44px] md:w-[44px] rounded-lg me-4 h-[38px] w-[38px]"
                  alt="Album Art"
                />
              ...
              {props.isSearch && (!props.seeds.includes(track.id) ? (
                <button
                  onClick={() =>
                    typeof props.handleClick === 'function' && props.handleClick(track.id)
                  }
